    ...
    this.a, this.windowMud = new window_mud, this.Salvar = function() {
        ...
        if ("" == e.value || "" == t.value || "" == o.value || "" == a || "" == r)(new window_mud).Alert("ATENÇÃO", "Preencha todos os campos.", "Formulário", "fas fa-exclamation", "w-p-default", !1, "");
            ...
            $.ajax({
                ...
                error: function(e) { 
                    console.log(e), (new window_mud).Alert("ALERTA", "Houve um erro em salvar.", "Banco de dados", "fas fa-times", "w-p-default", !1, "")
                }
            }).done(function(e) {
                (new window_mud).Alert("ALERTA", e, "Banco de dados", "fas fa-check", "w-p-default", !1, "");
                document.querySelector("#submitUS").disabled = !0;
                let t = Usuario.a,
                    o = document.querySelector("#cpf + span"),
                ...
                a.value = "", n.value = "", l.value = "", d.value = "", m.value = "", o.textContent = "cpf:" + t.cpf, r.textContent = "nome:" + t.nome, s.textContent = "email:" + t.email, u.textContent = "celular:" + t.celular, c.textContent = "senha:" + t.senha
            })
        }
    }, this.Excluir = function() {
        if(localStorage.getItem('login')){
            let pp = new window_mud;
            pp.Confirm('ALERTA','Você deseja excluir o seu Usuário? Lembre-se que ao excluir a sua conta, não poderá utilizar o MUD.','Usuário','far fa-user','w-p-default','#respExc');
            setTimeout(function(){console.log(pp.ReturnConfirm());if(pp.ReturnConfirm())document.location ='./api/index.php?A=5';},3000);} 
       else
        ...
        $.ajax({
            ...
            error: function(e) {
                console.log(e), (new window_mud).Alert("ALERTA", "Erro em pegar seus dados como usuário. Provavelmente há um erro no banco.", "Banco de dados", "far fan-bar", "w-p-default", !1, "")
            }
        }).done(function(n) {
